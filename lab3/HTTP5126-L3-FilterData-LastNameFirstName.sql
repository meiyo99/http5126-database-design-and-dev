...
-- B
SELECT item FROM stock_item WHERE price BETWEEN 10 AND 20;

--  4
-- A
SELECT CONCAT(first_name, " ", last_name) AS 'name', phone FROM employee WHERE NOT (role = 'Manager' OR role = 'Assistant');
-- B
SELECT item, price, inventory FROM stock_item WHERE NOT (category = 'Feline' OR category = 'Canine' OR inventory > 20);
